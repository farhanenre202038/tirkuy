...
<html lang="en">
  ...
  <body>
    ...
    <div class="container">
        ...
        <button id="get-user-btn">Random</button>
      </div>
...
let manualDataArray = [
    ...
    { caption: "Creative Endeavor" },
    { caption: "Joyful Exploration" },
    { caption: "Harmonious Journey" },
    { caption: "Whimsical Discovery" },
...
let getUser = () => {
    // Use the manual data from the array
    const currentData = manualDataArray[currentIndex];

    imgContainer.innerHTML = `<img src="img/profil.jpg">`;
    details.innerHTML = `
        <h2>tirr.default.my.id</h2>
        <h3>Made With 🖤 Haan</h3>
        <a href=""><i class="fa-solid fa-location-dot">${currentData.caption}</i></a>`;

    let randomColor =
        "#" + ((Math.random() * 0xffffff) << 0).toString(16).padStart(6, "0");
    document.documentElement.style.setProperty("--theme-color", randomColor);

    // Increment index or reset to 0 if reached the end of the array
    currentIndex = (currentIndex + 1) % manualDataArray.length;
};
