<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Asisten 🤖</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons"rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Raleway:wght@700&display=swap" rel="stylesheet">
    <link rel="icon" href="data:image/png;base64," type="image/png">

</head>
    <style>
         * {
        padding: 0;
        margin: 0;
        box-sizing: border-box;
        font-family: "Raleway", sans-serif;
        }
        .custom-button{
            padding: 20px;  
            border-radius: 50px;
        }
        .card{
            top:50px;
        }
        #recognition-result{
            margin-bottom: 20px;
        }
        .shadow{
            box-shadow: 2px 0px 10px 0px
        }
    </style>
<body>
    <?php require('template/navbar.php'); ?>
    <div class="container">
        <div class="card shadow">
           <div class="card-body p-5">
           <center>
                <h4 id="recognition-result"></h4>
                <div id="play"></div>
                <div id="stop"></div>
            </center>
           </div>
        </div>
    </div>

    <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.12.9/dist/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
    <script type="module">
         import { GoogleGenerativeAI } from "https://esm.run/@google/generative-ai";
        var button = document.getElementById("play");
        var buttonstop = document.getElementById("stop");
        button.innerHTML = `<button class="btn btn-danger custom-button"><span class="material-icons">mic</span></button>`;
        const API_KEY = "AIzaSyA4kp656ozNAJiy85bc4aNmBbtEnQtPusg";  
        const genAI = new GoogleGenerativeAI(API_KEY);

        async function run(query) {
            const model = genAI.getGenerativeModel({ model: "gemini-pro"});
            const prompt = query;
            const data = await model.generateContent(prompt, { max_words: 20 });
            //const { totalTokens } = await model.countTokens(prompt);
            const response = await data.response;
            const text = response.text();
            console.log(text);
            speakText(text);           
        }

        function speakText(text) {
            if ('speechSynthesis' in window) {
                var synth = window.speechSynthesis;
                var utterance = new SpeechSynthesisUtterance(text);
                utterance.lang = 'id-ID';
                utterance.addEventListener('start', function () {
                    button.innerHTML = `<button id="play" class="btn btn-danger custom-button" style="display:none;"><span class="material-icons">mic</span></button>`;
                    buttonstop.innerHTML = `<button class="btn btn-info custom-button"><span class="material-icons">stop</span></button>`;                    
                });
                utterance.addEventListener('end', function () {
                    buttonstop.innerHTML = `<button class="btn btn-info custom-button" style="display:none;"><span class="material-icons">stop</span></button>`;
                    button.innerHTML = `<button id="play" class="btn btn-danger custom-button"><span class="material-icons">mic</span></button>`;     
                });
                synth.speak(utterance);
            } else {
                console.error('Browser tidak mendukung Web Speech API');
            }
        }

        function startListening() {
            // Periksa apakah browser mendukung Web Speech API
            if ('webkitSpeechRecognition' in window) {
                var recognition = new webkitSpeechRecognition();

                // Meminta izin mikrofon saat fungsi dijalankan
                navigator.mediaDevices.getUserMedia({ audio: true })
                .then(function(stream) {
                    // Setelah izin diberikan, barulah mengatur sisa konfigurasi pengenalan suara
                    recognition.lang = 'id-ID';
                    recognition.maxDuration = 10000;

                    recognition.onstart = function() {
                        button.innerHTML = `<button class="btn btn-secondary custom-button"><span>Mendengar....</span></button>`;
                        var typing =  document.getElementById("recognition-result")
                        typing.textContent = ""; 
                    };

                    recognition.onresult = function(event) {
                        var result = event.results[0][0].transcript;
                        var typing =  document.getElementById("recognition-result")
                        typing.textContent = result; 
                    
                        switch (true) {
                            case result.toLowerCase().includes("buka youtube"):
                                window.open("https://www.youtube.com", "_blank");
                                speakText("Membuka YouTube");
                                break;
                            
                            case result.toLowerCase().includes("buka google"):
                                window.open("https://www.google.com", "_blank");
                                speakText("Membuka Google");
                                break;
                            
                            case result.toLowerCase().includes("wikipedia"):
                                var searchQuery = result.replace(/wikipedia/i, "").trim();
                                window.open("https://id.wikipedia.org/wiki/" + encodeURIComponent(searchQuery), "_blank");
                                speakText("Membuka Wikipedia" + searchQuery);
                                break;
                            
                            // case result.toLowerCase().includes("hello"):
                            //     var searchQuery = result.replace(/hello/i, "").trim();
                            //     speakText("Hai, Nama Saya Tirrbot ada yang bisa saya bantu ?");
                            //     break;

                            case result.toLowerCase().includes("hai tiara"):
                                var searchQuery = result.replace(/hai farhan/i, "").trim();
                                var query = searchQuery;
                                run(query);
                                break;

                            case result.toLowerCase().includes("jam"):
                                var currentTime = new Date();
                                var hours = currentTime.getHours();
                                var minutes = currentTime.getMinutes();
                                speakText("Sekarang Jam" + hours + "Lewat" + minutes + "Menit");
                                break;
                                
                            case result.toLowerCase().includes("tanggal"):
                                var currentTime = new Date();
                                var day = currentTime.getDate();
                                var month = currentTime.getMonth() + 1; // Bulan dimulai dari 0 (Januari)
                                var year = currentTime.getFullYear();
                                speakText("Hari Ini Tanggal" + day + "Bulan" + month + "Tahun" + year);
                                break;
                            
                            case result.toLowerCase().includes("play music"):
                                window.open("https://www.youtube.com/watch?v=ofrKud_dN40", "_blank");
                                speakText("Memutar Music Di Youtube");
                                break;
                            
                            default:
                                // Kasus default, jika tidak ada perintah yang cocok
                                speakText("Maaf Saya Tidak Paham Fitur Dalam Pengembangan");
                                break;
                        }
                    };

                    recognition.start();
                })
                .catch(function(err) {
                    console.error('Izin mikrofon tidak diberikan atau terjadi kesalahan', err);

                    // Tampilkan pesan kepada pengguna bahwa izin mikrofon diperlukan
                    alert('Aplikasi memerlukan izin mikrofon untuk berfungsi. Mohon izinkan akses mikrofon.');
                });
            } else {
                console.error('Browser tidak mendukung Web Speech API');
                
                // Tampilkan pesan kepada pengguna bahwa browser tidak mendukung fitur ini
                alert('Browser Anda tidak mendukung fitur pengenalan suara. Silakan gunakan browser yang mendukung, seperti Google Chrome.');
            }
        }

        button.addEventListener("click", function() {
            startListening();
        });

        buttonstop.addEventListener("click", function() {
            console.log("Berhasil");
        });
    </script>
</body>
</html>