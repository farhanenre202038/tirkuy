<!doctype html>
<html lang="en">
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
    <!-- Font Text -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Comfortaa:wght@300;600&family=Press+Start+2P&family=Silkscreen&display=swap" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Raleway:wght@700&display=swap" rel="stylesheet">
    <link rel="icon" href="data:image/png;base64," type="image/png">
   
    <script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js"></script>
    <title>TIRRSHOP🛒</title>
  </head>

    <style>
         * {
        padding: 0;
        margin: 0;
        box-sizing: border-box;
       
        }
        nav{
            font-family: "Raleway", sans-serif;
            background-color: #4e7bb4;
        }
        body{
            background-color: #343a40!important;
            font-family: "Silkscreen", sans-serif;
        }
        .card, .card-img-top{
            background-color: #343a40!important;
            box-shadow: 5px 5px 10px 0px black;
            color: white;
            border-radius: 20px;
        }

          .right-bottom-button {
            position: fixed;
            bottom: 20px;
            right: 20px;
            padding: 10px 20px;
            background-color: #4e7bb4;
            color: #fff;
            border: none;
            border-radius: 100px;
            cursor: pointer;
            font-size: 20px;
          }
          
          .right-bottom-button:hover {
            background-color:  #58769c;
            cursor: pointer;
          }


          .cart {
            display: none;
            position: fixed;
            bottom: 70px;
            right: 20px;
            padding: 20px;
            background-color: #fff;
            border: 1px solid #ccc;
            border-radius: 5px;
            margin-bottom: 10px;
            max-height: 400px; /* Set maksimum tinggi cart */
            overflow-y: auto; /* Tambahkan overflow untuk scroll jika kontennya melebihi tinggi maksimum */
          }

          .quantity-badge{
            display: inline-block;
            padding: 1px 5px;
            background-color: red;
            border-radius: 6px;
            font-size: 0.8rem;
            position: absolute;
            top: 0;
            right: -5px;
          }

                    /* ... (CSS yang sama seperti sebelumnya) */

          .cart-item {
            display: flex;
            align-items: center;
            margin-bottom: 10px;
            border: 1px solid black;
            padding: 10px;
            border-top: none;
            border-right: none;
            border-left: none;
          }

          .cart-item img {
            width: 50px;
            height: 50px;
            margin-right: 10px;
          }

          .cart-item p {
            margin: 0;
          }

         #add,
         #remove{
            display: inline-block;
            padding: 2px 5px;
            cursor: pointer;
            margin: 0 8px;
            background-color: black;
            color: white;
            font-weight: bold;
            font-size: 0.8rem;
            border-radius: 5px;
          }

          hr{
            background-color: black;
          }

          .btn-success.disabled, .btn-success:disabled{
            border: none;
            background-color: #8a8888;
            cursor: not-allowed;
          }

          .form-control{
            font-family: monospace;
          }

          @media screen and (max-width: 992px) {
            .col-md-3{
              width: 100%;
              min-height: 0px;
              padding-right: 0px;
              padding-left: 0px;
              max-width: 50%;
            }
          }
    </style>

  <body>
     <!-- navbar -->
     <?php require('template/navbar.php'); ?>
    <!-- navbar -->
    <div class="jumbotron jumbotron-fluid">
      <div class="container">
        <h1>TIRRSHOP🛒</h1>
        <p class="lead">Kreativitas Tanpa Batas, Pelayanan Tanpa Kompromi.</p>
      </div>
    </div>
    <div class="container" x-data="produk">
        <h3 class="text-center mt-3 text-white"><span>Produk Kami</span></h3>
        <div class="row no-gutters">
           <template x-for="(items, index) in item" x-key="index">
                <div class="col-md-3">
                    <div class="card m-3">
                        <img class="card-img-top" :src="`img/${items.img}`" :alt="items.nama">
                        <div class="card-body">
                          <p class="card-title"><span x-text="items.nama"></span></p>
                          <h6 class="card-text"><b><span x-text="rupiah(items.harga)"></span></b></h6>
                        </div>
                        <div class="card-body">
                          <button @click.prevent="$store.cart.add(items)" type="button" class="btn btn-secondary btn-block">🛒</button>
                        </div>
                      </div>
                </div>
           </template>
        </div>
    </div>
   
    <section x-data>
         <!-- Button Keranjang -->
    <button onclick="toggleCart()" class="right-bottom-button">🛒<span class="quantity-badge" x-show="$store.cart.quantity" x-text="$store.cart.quantity"></span></button>
    <!-- keranjang -->
    <div id="shopping-cart" class="cart">
        <!-- Isi cart di sini -->
        <template x-for="(items, index) in $store.cart.item" x-keys="index">
          <div class="cart-item">
            <img :src="`img/${items.img}`" :alt="items.nama">
            <div class="item-details">
              <p x-text="items.nama"></p>
              <span x-text="rupiah(items.harga)"></span> &times;
              <button id="remove"  @click="$store.cart.remove(items.id)">&minus;</button>
              <span x-text="items.quantity"></span>
              <button id="add" @click="$store.cart.add(items)">&plus;</button> &equals;
              <span x-text="rupiah(items.total)"></span>
            </div>
          </div>
        </template>
        <h5 x-show="!$store.cart.item.length"><b>Your shopping cart is empty.</b></h5>
        <center>
          <h5 x-show="$store.cart.item.length"><b>Total : <span x-text="rupiah($store.cart.total)"></span></b></h5>
          <span x-show="$store.cart.item.length"><hr></span>
       </center>
      <div class="form-container" x-show="$store.cart.item.length">
        <form action="" id="checkoutForm">
          <input type="hidden" name="item" x-model="JSON.stringify($store.cart.item)">
          <input type="hidden" name="total" x-model="$store.cart.total">
          <h5 class="text-center">Customer Detail</h5>
          <div class="form-group">
            <input type="text" name="nama" id="nama" class="form-control form-control-sm" placeholder="Full Name" autocomplete="off">
          </div>
          <div class="form-group">
            <input type="email" name="email" id="email" class="form-control form-control-sm" placeholder="Email" autocomplete="off">
          </div>
          <div class="form-group">
            <input type="number" name="phone" id="phone" class="form-control form-control-sm" autocomplete="off" placeholder="WhatsApp">
          </div>

          <button class="btn btn-success btn-block disabled" type="submit" id="checkout-button" value="checkout">Checkout</button>
        </form>
      </div>
      </div>
    </section>

  
    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.12.9/dist/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
    <script src="src/app.js"></script>
  </body>
</html>